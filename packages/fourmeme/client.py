...
class FourMemeClient:
    ...
    async def upload_image(self, image_path: str | Path) -> str:
        """Upload a token logo image from a file path. Returns CDN URL."""
        path = Path(image_path)
        with open(path, "rb") as f:
            image_bytes = f.read()
        mime = mimetypes.guess_type(str(path))[0] or "image/png"
        return await self.upload_image_bytes(image_bytes, mime=mime, filename=path.name)

    async def upload_image_bytes(self, image_bytes: bytes, mime: str = "image/png", filename: str = "token.png") -> str:
        """Upload a token logo image from raw bytes. Returns CDN URL."""
