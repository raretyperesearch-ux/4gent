...
function App() {
  ...
  async function startLaunch() {
    ...
    try {
      ...
      // ── Step 3: MetaMask calls newToken on Flap portal ────────────────────────
      ...
      // Encode newToken(string name, string symbol, string meta)
      // selector = 0x5bb8f4b6 (keccak256 of "newToken(string,string,string)")[0:4]
      // We'll encode this manually using hex
      const encodeString = (str) => {
        ...
        dataHex = dataHex.padEnd(padded * 2, "0");
        return lenHex + dataHex;
      };

      const nameEnc   = encodeString(form.name);
      const symbolEnc = encodeString(form.ticker);
      const metaEnc   = encodeString(prep.meta_url);

      // Offsets: 3 dynamic strings, offsets are 0x60, 0x60+len(name_enc), etc.
      const nameOffset   = (3 * 32).toString(16).padStart(64, "0");
      const nameSize     = 32 + Math.ceil(form.name.length / 32) * 32;
      const symbolOffset = (3 * 32 + nameSize).toString(16).padStart(64, "0");
      const symbolSize   = 32 + Math.ceil(form.ticker.length / 32) * 32;
      const metaOffset   = (3 * 32 + nameSize + symbolSize).toString(16).padStart(64, "0");

      const calldata = "0x5bb8f4b6" +
        nameOffset + symbolOffset + metaOffset +
        nameEnc + symbolEnc + metaEnc;
      ...
      // ── Step 4: Tell backend the tx hash ─────────────────────────────────────
