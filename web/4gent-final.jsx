...
const POSTS = {
  degen:      (n,t) => [
    { type:"INIT",     text:`AGENT ${n.toUpperCase()} ONLINE. $${t} IS THE SIGNAL. WATCHING FOUR.MEME. WHEN I SEE A PLAY — YOU'LL KNOW FIRST. ◈` },
...
const LOGS = [
  { t:300,  msg:"INITIALIZING AGENT RUNTIME",            ok:false },
  { t:700,  msg:"GENERATING IDENTITY MATRIX VIA CLAUDE", ok:false },
  { t:1200, msg:"IDENTITY LOCKED",                       ok:true  },
  { t:1600, msg:"UPLOADING TOKEN IMAGE TO FOUR.MEME CDN",ok:false },
  { t:2100, msg:"IMAGE STORED",                          ok:true  },
  { t:2500, msg:"REQUESTING createToken() ARGS",         ok:false },
  { t:3000, msg:"SUBMITTING TRANSACTION ON BSC",         ok:false },
  { t:3500, msg:"CONFIGURING TAX — 3% → TREASURY",      ok:false },
  { t:4000, msg:"TOKEN DEPLOYED — 0x7F3A...D91C",        ok:true  },
  { t:4400, msg:"VERIFYING @4GENTBOT IS CHANNEL ADMIN",  ok:false },
  { t:4900, msg:"CHANNEL VERIFIED",                      ok:true  },
  { t:5300, msg:"DISPATCHING INTRO POSTS",               ok:false },
  { t:5700, msg:"POSTS SENT",                            ok:true  },
  { t:6100, msg:"STARTING FOUR.MEME WATCHER",            ok:false },
  { t:6500, msg:"MONITOR ACTIVE",                        ok:true  },
  { t:6900, msg:"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", ok:null  },
  { t:7200, msg:"AGENT ONLINE",                          ok:true, final:true },
];
...
function App() {
  ...
  const [verifyStatus, setVerifyStatus] = useState("idle"); // idle | checking | ok | fail
    ...
    <div style={{minHeight:"100vh",background:"#F5F2EA",color:"#0A0A0A",fontFamily:M,display:"flex",flexDirection:"column",position:"relative"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .mb{transition:all .1s;cursor:pointer;padding:10px 18px;font-family:'Share Tech Mono',monospace;font-size:9px;letter-spacing:2px;text-align:center}
      `}</style>
      ...
      <div className="r" style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"40px 44px",background:"transparent"}}>
          ...
          {step===0&&<div className="fu">
            ...
            <div style={{fontFamily:M,fontSize:10,color:"#A89868",marginBottom:6,letterSpacing:2}}>LAUNCH A TELEGRAM ASSISTANT OR AUTONOMOUS AGENT IN 60 SECONDS.</div>
            <div style={{fontFamily:M,fontSize:9,color:"#C8C0A0",marginBottom:36,letterSpacing:1}}>3 FIELDS. CLAUDE HANDLES THE REST.</div>

            <div style={{maxWidth:500,display:"flex",flexDirection:"column",gap:22}}>
              <div><Label>AGENT NAME</Label><input placeholder="DegenDave / CoffeBot / AlphaMaxx..." value={form.name} onChange={e=>upd("name",e.target.value)}/></div>
          ...
          {step===1&&<div className="fu">
            ...
            <div style={{maxWidth:520,display:"flex",flexDirection:"column",gap:24}}>
              <div>
                ...
                <div style={{fontFamily:M,fontSize:8,color:"#C8C0A0",marginTop:5}}>{form.prompt.length} CHARS — MORE DETAIL = SHARPER AGENT</div>
              </div>
          ...
          {/* 03 TELEGRAM — fixed: manual setup flow */}
          {step===2&&<div className="fu">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,maxWidth:720}}>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                <div style={{padding:"10px 14px",border:"1px solid #E8D898",background:"#FDF8E8",marginTop:4}}>
                  <div style={{fontFamily:M,fontSize:9,color:"#908030",lineHeight:1.7}}>
                    WHY YOU CREATE IT — TELEGRAM BOTS CANNOT CREATE CHANNELS.<br/>
                    <span style={{color:"#C0A850"}}>YOUR CHANNEL. YOUR COMMUNITY. WE JUST POWER THE AGENT.</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {posts.length>0 ? posts.map((p,i)=>(
                  <div key={i} onClick={()=>setPreviewIdx(i)} style={{
                    ...
                    <div style={{display:"flex",gap:8,marginBottom:5}}>
                      ...
                      <div style={{fontFamily:M,fontSize:7,color:"#C8C0A0"}}>POST {i+1}</div>
                    </div>
                ...
                {posts.length>0&&<div style={{fontFamily:M,fontSize:7,color:"#C8C0A0"}}>PREVIEWS ONLY. ACTUAL POSTS WILL BE SMARTER.</div>}
              </div>
            </div>
          </div>}
            ...
            <div style={{maxWidth:460,display:"flex",flexDirection:"column",gap:12}}>
              <div onClick={()=>upd("trading",!form.trading)} style={{
                ...
                <div>
                  ...
                  <div style={{fontFamily:M,fontSize:8,color:"#C0B888",marginTop:3}}>AGENT EXECUTES TRADES ON FOUR.MEME</div>
                </div>
              ...
              {form.trading&&<div className="fu" style={{display:"flex",flexDirection:"column",gap:10}}>
                <div style={{padding:"10px 14px",border:"1px solid #E8C8A0",background:"#FDF6EC",fontFamily:M,fontSize:9,color:"#908030",lineHeight:1.7}}>
                  ◈ AGENT TRADES FROM YOUR WALLET.<br/><span style={{color:"#C0A040"}}>4GENT NEVER HOLDS YOUR FUNDS.</span>
                </div>
          ...
          {/* 05 LAUNCH — fixed: wallet connect + image confirm + fee ack */}
          {step===4&&<div className="fu">
            ...
            <div style={{fontFamily:M,fontSize:10,color:"#A89868",marginBottom:32,letterSpacing:2}}>THREE THINGS BEFORE WE DEPLOY.</div>
                ...
                <Card>
                  {[
                    ...
                    <div key={k} style={{display:"flex",padding:"8px 14px",borderBottom:"1px solid #F0E8D8",gap:16}}>
                      <div style={{fontFamily:M,fontSize:7,color:"#C0B888",width:100,flexShrink:0,letterSpacing:1}}>{k}</div>
              ...
              <div>
                ...
                <div className="ck" onClick={()=>setFeeAck(!feeAck)}>
                  ...
                  <div style={{fontFamily:M,fontSize:9,color:"#605848",lineHeight:1.7}}>
                    I UNDERSTAND LAUNCHING COSTS ~0.02 BNB IN GAS.<br/>
                    <span style={{color:"#A89868"}}>TAX FEES ONLY ACTIVATE AFTER GRADUATION TO PANCAKESWAP. MOST TOKENS DO NOT GRADUATE — LAUNCH FOR THE SOCIAL LAYER, NOT GUARANTEED FEES.</span>
                  </div>
                </div>
              </div>
                ...
                <div ref={logsRef} style={{border:"1px solid #1A1A0A",background:"#0A0A08",padding:"16px",maxHeight:220,overflowY:"auto"}}>
                  {logs.map((l,i)=>(
                    <div key={i} className="le" style={{
                      ...
                      {l.msg}
                      {l.ok===true&&<span style={{marginLeft:"auto",color:"#C9A84C44",fontSize:7,letterSpacing:2}}>OK</span>}
                    </div>
                  ))}
          ...
          {step===5&&launched&&<div className="fu">
            ...
            <div style={{fontFamily:M,fontSize:10,color:"#A89868",marginBottom:28,letterSpacing:2}}>
              {(form.name||"AGENT").toUpperCase()} IS LIVE. WATCHING. POSTING. NEVER SLEEPING.
            </div>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,maxWidth:760}}>

              <Card style={{display:"flex",flexDirection:"column"}}>
                <div style={{padding:"10px 14px",borderBottom:"1px solid #F0E8D8",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div>
                    <div style={{fontFamily:M,fontSize:9,color:"#A89868"}}>t.me/{handle}</div>
                    <div style={{fontFamily:M,fontSize:7,color:"#C8C0A0",marginTop:1}}>PUBLIC CHANNEL · 1 MEMBER</div>
                  </div>
                  <div style={{fontFamily:M,fontSize:7,color:"#5DB870",letterSpacing:2}}>● LIVE</div>
                </div>
                <div style={{padding:"10px",display:"flex",flexDirection:"column",gap:6,flex:1}}>
                  {posts.map((p,i)=>(
                    <div key={i} style={{padding:"10px 12px",background:"#fff",border:"1px solid #F0EAE0"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                        {imgPreview
                          ? <img src={imgPreview} style={{width:18,height:18,borderRadius:"50%",objectFit:"cover"}}/>
                          : <span style={{fontFamily:M,fontSize:14,color:G}}>{arch?.icon}</span>
                        }
                        <span style={{fontFamily:M,fontSize:9,color:"#1A1A0A"}}>{form.name}</span>
                        <span style={{fontFamily:M,fontSize:7,color:"#C8C0A0",marginLeft:"auto"}}>{["JUST NOW","1M AGO","2M AGO"][i]}</span>
                      </div>
                      <div style={{fontFamily:M,fontSize:9,color:"#605848",lineHeight:1.6}}>{p.text}</div>
                    </div>
                  ))}
                </div>
                <div style={{padding:"10px 14px",borderTop:"1px solid #F0E8D8",fontFamily:M,fontSize:9,color:G,cursor:"pointer"}}>
                  ◈ SHARE t.me/{handle}
                </div>
              </Card>
              ...
              <Card style={{display:"flex",flexDirection:"column"}}>
                ...
                <div style={{padding:"16px 14px",flex:1,display:"flex",flexDirection:"column",gap:14}}>
                  ...
                  <div style={{fontFamily:M,fontSize:9,color:"#A89868",lineHeight:2}}>
                    AFTER CLAIMING:<br/>
                    <span style={{color:"#C0B070"}}>/stats /fees /pause /resume<br/>/updateprompt /frequency<br/>/tglink</span>
                  </div>
                </div>
              </Card>
              ...
              <div style={{gridColumn:"1/-1",display:"grid",gridTemplateColumns:"repeat(4,1fr)",border:"1px solid #E8E0C8",background:"#FEFCF5"}}>
                {[
                  ...
                  ["BONDING CURVE","0%",              "JUST DEPLOYED"],
                  ["HOLDERS",      "1",               "GROWING"],
                  ["DAILY CUT",    "0 BNB",           "GROWS WITH VOLUME"],
                ].map(([l,v,s],i)=>(
