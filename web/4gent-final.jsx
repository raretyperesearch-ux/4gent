...
function App() {
  ...
  async function startLaunch() {
    ...
    try {
      ...
      // ── Step 4: Compute vanity salt (address must end in 8888) ─────────────
      addLog("COMPUTING VANITY SALT...");
      const PORTAL     = "0xe2cE6ab80874Fa9Fa2aAE65D277Dd6B8e65C9De0";
      const TOKEN_IMPL = "0x8b4329947e34b6d56d71a3385cac122bade7d78d";

      // EIP-1167 minimal proxy bytecode hash for this tokenImpl
      const proxyBytecode = "3d602d80600a3d3981f3363d3d373d3d3d363d73"
        + TOKEN_IMPL.slice(2).toLowerCase()
        + "5af43d82803e903d91602b57fd5bf3";
      const bytecodeHash = ethers.keccak256("0x" + proxyBytecode);

      // CREATE2 address prediction
      const predictAddress = (saltBytes32) => {
        // pack: 0xff ++ portal(20) ++ salt(32) ++ bytecodeHash(32) = 85 bytes
        const portalBytes  = ethers.getBytes(PORTAL);           // 20 bytes
        const saltBytes    = ethers.getBytes(saltBytes32);       // 32 bytes
        const hashBytes    = ethers.getBytes(bytecodeHash);      // 32 bytes
        const packed       = new Uint8Array(1 + 20 + 32 + 32);
        packed[0]          = 0xff;
        packed.set(portalBytes, 1);
        packed.set(saltBytes,   21);
        packed.set(hashBytes,   53);
        const result = ethers.keccak256(packed);
        return "0x" + result.slice(26); // last 20 bytes (skip 0x + 12 bytes = 26 chars)
      };

      let salt;
      let attempts = 0;
      let currentSalt = ethers.randomBytes(32);
      while (true) {
        const saltHex = ethers.hexlify(currentSalt);
        const predicted = predictAddress(saltHex);
        if (predicted.toLowerCase().endsWith("8888")) {
          salt = saltHex;
          break;
        }
        currentSalt = ethers.getBytes(ethers.keccak256(currentSalt));
        attempts++;
        if (attempts % 5000 === 0) addLog(`SALT SEARCH... ${attempts} attempts`);
      }
      addLog(`SALT FOUND (${attempts} attempts) ✓`, true);

      // ── Step 5: MetaMask calls newTokenV5 on Flap portal ─────────────────────
      ...
      // newTokenV5 — exact field order from official ABI
      // standard token: taxRate=0, migratorType=1 (V3), dexThresh=3 (FOUR_FIFTHS=80%)
      const PORTAL_ABI = [{
        "name": "newTokenV5",
        "type": "function",
      ...
      const params = [
        ...
        form.ticker,
        ipfsCid,
        3,                                                                    // dexThresh: FOUR_FIFTHS
        salt,                                                                 // vanity salt
        0,                                                                    // taxRate: 0 = standard token
        1,                                                                    // migratorType: 1 = V3
        "0x0000000000000000000000000000000000000000",                        // quoteToken: native BNB
        0,                                                                    // quoteAmt
        wallet,                                                               // beneficiary: launcher gets rev share
        "0x",                                                                 // permitData
      ...
      const calldata = iface.encodeFunctionData("newTokenV5", [params]);
      ...
      // ── Step 6: Tell backend the tx hash ─────────────────────────────────────
