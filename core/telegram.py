...
PLATFORM_BOT_TOKEN = lambda: os.environ["PLATFORM_BOT_TOKEN"]
...
async def set_bot_photo(bot_token: str, image_url: str) -> bool:
    """Download image from URL and set as bot profile photo."""
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            img = await client.get(image_url)
            img.raise_for_status()
            url = TG_API.format(token=bot_token, method="setMyPhoto")
            r = await client.post(url, files={"photo": ("photo.jpg", img.content, "image/jpeg")})
            return r.json().get("ok", False)
    except Exception as e:
        logger.warning("Set bot photo failed: %s", e)
        return False
...
async def assign_bot_from_pool(supabase: Client, agent_id: str) -> Optional[dict]:
    ...
    try:
        ...
        logger.info("Assigned bot @%s to agent %s", bot["bot_username"], agent_id)
        return bot
...
async def handle_owner_command(update: dict, supabase: Client) -> None:
    """
